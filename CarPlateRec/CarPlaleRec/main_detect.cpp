...
int getThreshVal_Otsu_8u( IplImage* _src )
{
    ...
    double mu = 0.0, scale = 1.0/(width*height);
    for( i = 0; i < N; i++ )
        mu += i*(double)h[i];
    
    mu *= scale;
    double mu1 = 0.0, q1 = 0.0;
    double max_sigma = 0.0, max_val = 0.0;
...
IplImage* myCanny(IplImage* img)
{
    ...
    int weak_edge_threshold = int(strong_edge_threshold*0.4f);
...
{
	double num = 0;
    int h = int(img->height/**0.5*/ - img->height*0.125f);
