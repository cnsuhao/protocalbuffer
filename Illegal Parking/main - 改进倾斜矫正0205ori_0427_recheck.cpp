...
bool IniRead(char strfind[], char strget[], int nStrgetSize, char* section)
{	
	return GetPrivateProfileString(section, strfind, "", strget, nStrgetSize, ".\\run.ini") > 0;
...
bool IniWrite(char strKey[], char strValue[], char* section)
{
	return WritePrivateProfileString(section, strKey, strValue, ".\\run.ini")==TRUE;
}
		...
		if(style!=1)
		{
			for(int i=start;i<=charnum;i++)
			{
				...
				if(f!=1&&i==7&&f!=3)//非蓝牌末位
				{
					...
					if(f == 0)
						Nnw.load("xmlcar/mlp_blackback1215.xml");
					if(f == 2)
						Nnw.load("xmlcar/mlp_whiteback1215.xml");
					if(f == 3)
						Nnw.load("xmlcar/mlp_yellowback1215.xml");
				...
				if(i==2)
				{
					...
					if(ch=='D'||ch=='0'||ch=='O')
					{
						...
						Nnw0d.load("xmlcar/0D_dis_364_2258.xml");
						vector<float> feat;
		...
		if(exflag==0)
		{
			...
			if(style!=1)
			{
				...
				if(f==2)//白牌，有10个字母
				{
					...
					Nnw1.load("xmlcar/mlpchar_white_head.xml");
				...
				else//蓝，黄，黑
				{
					...
					CvANN_MLP Nnw1(layerSizes1, CvANN_MLP::SIGMOID_SYM,0, 0 );
					Nnw1.load("xmlcar/char_0207_v2.xml");//================================
					Nnw1.predict(cf1,result1);
				}
	...
	vector<Rect> &rt, int &rete , vector<string>&Carplates, vector<string>&Platecolor, vector<Rect>&Positions )
{
	...
	for(int i=0;i<finalout.size();i++)//处理得出结果
	{
		...
		else
		{
			int max=1;
			int pos=0;
			for(int k=0;k<Carplates.size();k++)
			{
				int cnt=1;
				for(int n=k+1;n<Carplates.size();n++)
				{
...
int detect_pic(Mat matScr  ,vector<string>&Carplates , vector<string>&Platecolor, vector<Rect> &Positions  )
{
	...
	if(retest==0)//ori
	{
		exnum = ExtractPlate(matScr, matCopy, matGray, i,count,exnum,1,retest , Carplates , Platecolor ,Positions );			
	}
	if( retest == 1 )//lbp
	{
		//gra = gray1.clone();		
...
bool CarPlateRecognition(char Filepath[], vector<string>&Carplates , vector<string>&Platecolor, vector<Rect> &Positions, bool model )
{
 ...
 { 
	 ...
	 if (!PathFileExists(savepath))//文件夹不存在则创建by hb at 2016.03.18
	 {
		 SHCreateDirectoryEx(NULL, savepath, NULL);
	 }
