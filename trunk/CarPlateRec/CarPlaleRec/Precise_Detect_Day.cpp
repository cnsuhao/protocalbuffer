...
int Test_color(Mat img1)
{
	...
	sprintf_s(sName, sizeof(sName), "simple_result-sp-col/e_img.jpg");//保存图像
	imwrite(sName, img);
	Mat thresh = Mat(img.size(), CV_8UC1);
	Mat gray = Mat(img.size(), CV_8UC1);
	...
	sprintf_s(sName, sizeof(sName), "simple_result-sp-col/gray.jpg");//保存图像
	imwrite(sName, gray);

	float s = MatBackgroudValue(gray);//s<0.5是深底，否则是浅底
    ...
    g_NnwColor.predict(cf1, result1);
...
{
	char sName[300] = { 0 };
	sprintf_s(sName, sizeof(sName), "simple_result-sp-col/color_img .jpg");//保存图像
	imwrite(sName, img);
	Mat gray = Mat(img.size(), CV_8UC1);
	cvtColor(img, gray, CV_BGR2GRAY);
	sprintf_s(sName, sizeof(sName), "simple_result-sp-col/gray_img .jpg");//保存图像
	imwrite(sName, gray);
	...
	Nnw.load("xmlcar/mlp_color0115.xml");
	RGBfeature(img, charFeature1);
	bool back = 0;
	back = HSVfeature(img, charFeature1);
	Mat cf1 = Mat(1, 12, CV_32FC1);
...
void GapBlack(Mat  Pimg, Mat  thresh, vector<int>& Px, float Cx)
{
	...
	int max = 0;
	unsigned int n = 0;//max：最大的峰值； n:每次求得的最大值的位置
	for (unsigned int i = 0; i</*Pimg.cols*/P.size(); i++)//求最大值
	{
		if (P[i]>max)
		{
			max = P[i];
	...
	{
		max_once = 0, n = 0;
		for (unsigned int i = n; i</*Pimg.cols-1*/P.size(); i++)
		{
        ...
        if (max_once > max*0.92f)//最大值满足要求
		{
			int iLeftSide = 0, iRightSide = n;
            for (int i = n; i >= 0; i--)//左边界 这里i不能为unsigned类型 否则会无限循环
			{
				if (P[i] < max*Cx)
				{
					if (i < n)
			...
			for (unsigned int i = n; i</*Pimg.cols*/P.size(); i++)
			{
				if (P[i] < max*Cx)//右边界
				{
					if (i>n)
...
**返回值:返回车牌颜色 或 错误信息
*/
int Precise_detect(Mat plate_region, Mat color, bool whitebody, int colnum, int snum, int cons, int &st, int retest, vector<Mat> &block)
{
	...
	if (cons == 2)//双排结构确定上下边界
	{
		...
		if (scale<0.33)
		{
			int downl = (int)((iDownFoot - iUpFoot) * (1 - scale));
			iUpFoot = iDownFoot - (int)((iDownFoot - iUpFoot) * (1 - scale) * 1.55f);
			if (iUpFoot < 0)
				iUpFoot = 0;
			scale = (float)((iDownFoot - iUpFoot - downl) / (iDownFoot - iUpFoot));
		}
	}
	Py.clear();
	...
	if (dou_con == true)//双层结构车牌
	{
		...
		rect.height = (int)(plate.rows*scale);
		...
		//切出上半部分车牌
		rect.x = 0;
		rect.y = (int)(plate.rows*scale);
		rect.width = plate.cols;
		rect.height = (int)(plate.rows*(1 - scale));
		if (rect.x<0 || rect.y<0 || rect.width <= 0 || rect.height <= 0 || rect.x + rect.width>plate.cols || rect.y + rect.height>plate.rows)
			return  -1;
		...
		{
			rect.x = 0;
			rect.y = thresh_up.rows - (int)(0.6f*thresh_down.rows);
			rect.width = thresh_up.cols;
			rect.height = (int)(0.6*thresh_down.rows);
		...
		{
			Rect roi;
			roi.x = PlateSeg[1].cols - (int)(PlateSeg[1].rows*1.2f);
			roi.width = (int)(PlateSeg[1].rows*1.2);
			roi.y = 0;
		...
		if (PlateSeg[2].cols > PlateSeg[2].rows*1.2)
		{
			Rect roi;
			roi.x = 0;
			roi.width = (int)(PlateSeg[2].rows*1.2f);
			roi.y = 0;
		...
		{
			Rect roi;
			roi.x = PlateSeg[1].cols - (int)(PlateSeg[2].cols*1.1f);
			if (roi.x<0)
				roi.x = 0;
			roi.y = 0;
			roi.width = (int)(PlateSeg[2].cols*1.1f);
			roi.height = PlateSeg[1].rows;
	...
	if (dou_con == false && cons == 1)
	{
		...
		*/
		...
		float maxs = 0;
		int maxpos = 0;
        for (int i = int(cetd.size() - 1); i >= 1; i--)
		{
		...
		if (temp_v_pos.size() >= 3)//一般车牌
		{
			...
			{
				for (unsigned int i = 0; i <= temp_v_pos.size() - 3; i++)
				{
		...
		{
			for (unsigned int i = 0; i<temp_v_pos.size(); i++)
			{
				for (unsigned int j = i + 1; j<temp_v_pos.size(); j++)
				{
		...
		{
			int uncont = 0;
			for (unsigned int i = 0; i<temp_v_pos.size() - 1; i++)
			{
				if (temp_v_pos[i] != temp_v_pos[i + 1] + 1)
					uncont = 1;
			}
		...
		{
			//计算首字块与第二字块之间的间隙宽度
			int firstgap = (int)(((charSegment3[3][0] + charSegment3[3][1]) / 2 - (charSegment3[2][0] + charSegment3[2][1]) / 2)*(12.0f / 57.0f));
			//估计首字块左右边界
		...
		if (style != 1 && style != 3)
		{
			...
			for (unsigned int k = 0; k<Sx.size(); k++)
			{
		...
		if (style == 0)
		{
			...
			for (int i = 2; i <= 7; i++)
			{
				if (abs(PlateSeg[i].rows - aveheight) < min)
				{
					...
					min = (int)abs(PlateSeg[i].rows - aveheight);
				}
			}
			...
			{
				r.x = (int)(PlateSeg[1].cols - avewidth);
				r.y = 0;
				r.width = (int)avewidth;
				r.height = PlateSeg[1].rows;
				Mat ts1 = PlateSeg[1](r);
				PlateSeg[1] = ts1.clone();
			}
			if (PlateSeg[1].cols< avewidth)
			{
				r.x = (int)(charSegment3[0][1] - avewidth);
				r.y = 0;
				r.width = (int)avewidth;
				r.height = PlateSeg[1].rows;
				if (r.x<0)
			...
			if (PlateSeg[1].rows>aveheight)
			{
				...
				int up = 0;
				for (unsigned int a = 0; a<px1.size(); a++)
				{
					if (px1[a] != 0)
					{
						up = a;
