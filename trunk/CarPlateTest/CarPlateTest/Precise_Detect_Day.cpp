	...
	if(max_once > max*0.92)//最大值满足要求
		{
			...
			if(rfoot-lfoot==0)                                        //
			{
				//for(int i=lfoot; i<=rfoot; i++)//将这一范围清零
				//{					
				if(!Pimg.empty()&&!thresh.empty())
				{
					for(int j=0; j<Pimg.rows; j++)
					{	
							Pimg.at<uchar>(j, rfoot) = 0;
							thresh.at<uchar>(j, rfoot) = 255;				
					}
				}
					P[rfoot] = 0;
				//}
...
{	
	...
	if(cons == 1)//对单排结构确定上下边界
	{
		...
		for(int nnn=0; nnn<3; nnn++)//band筛选
		{
			...
			lfoot=0, rfoot=plate_region.rows-1;
			//在最大值附近根据阈值比例Cy找边界
			for(int i=n-1; i>=0; i--)
			{
				if(Py[i]<max*Cy)
			...
			if(plate_region.cols / (rfoot-lfoot) < 20)
				break;
			else//边界过小，则是干扰涂黑去除
			{
				for(int i=lfoot; i<=rfoot; i++)
					Py[i] = 0;
			}
		}
	}
	if(cons == 2)//双排结构确定上下边界
	{
		...
		for(int nnn=0; nnn<3; nnn++)//band筛选
		{
			...
			if(lfoot-4>=0)
				lfoot = lfoot-4;
			scale=(float)(minpos-lfoot)/(float)(rfoot-lfoot);
			if(scale<=0||scale>=1)
			{
				return -1;
		...
		//if(rfoot+2<plate_region.rows)
		//	rfoot=rfoot+2;
		if(scale<0.33)
		{
			int downl=(rfoot-lfoot)*(1-scale);
			lfoot=rfoot-(rfoot-lfoot)*(1-scale)*1.55;
			if(lfoot<0)
				lfoot=0;
			scale=(float)(rfoot-lfoot-downl)/(float)(rfoot-lfoot);
		}
	}
	Py.clear();
	...
	if(ROI_rect.x<0||ROI_rect.y<0||ROI_rect.width<=0||ROI_rect.height<=0||ROI_rect.x+ROI_rect.width>img0.cols||ROI_rect.y+ROI_rect.height>img0.rows)
		return  -1;
	Mat  band = plate_region(roi);
	Mat  img_band = img0(roi);
	...
	IplConvKernel *elem_close_vertical = cvCreateStructuringElementEx(an, an, (an-1)/2, (an-1)/2, CV_SHAPE_CROSS/*CV_SHAPE_ELLIPSE*//*CV_SHAPE_RECT*/, 0);
	...
	lfoot=0;int llfoot=0;rfoot=band.cols-1;n=band.cols*0.5;
	float l=0.37;/*0.37*/
	int m;
	...
	max=1; n=1; lfoot=0; rfoot=plate0.rows-1;	
	...
	ROI_rect2.height = rfoot - lfoot ;

	Rect ROI_rectf;
	ROI_rectf.x = 0;
	ROI_rectf.y = lfoot;
	ROI_rectf.width = band.cols;
	ROI_rectf.height = rfoot - lfoot ;
  	...
  	if(dou_con==true)//双层结构车牌
	{
		...
		{			
			...
			lfoot=0;rfoot=thresh_down.rows-1;			
			...
			if(rfoot-lfoot<0.6*thresh_down.rows)
			{
	...
	else if(cons == 1)
	{
		...
		for(n=0; n<band_clip.cols;)
		{
			...
			if(rfoot-lfoot<0.6*thresh0.rows)
			{
					for(int k=charSegment1[count][0]; k<=charSegment1[count][1]; k++)
					{
						for(int l=0; l<thresh0.rows; l++)
		...
		if((float)ave/(float)band_clip.rows>0.8)//长宽比满足这样的条件为高度多切
		{
			...
			reband.height=bhgt;
			if(reband.width<=0||reband.x<0||reband.width+reband.x>img0.cols||reband.height<=0||reband.y<0||reband.height+reband.y>img0.rows)
			{
				return -1;
			}
			Mat rb = img0(reband);
			...
			if(replate.width<=0||replate.x<0||replate.width+replate.x>img0.cols||replate.height<=0||replate.y<0||replate.height+replate.y>img0.rows)
			{
				return -1;
			}
			Mat o_plate = img0(replate);			
			plate=o_plate.clone();
			Rect r; 
			r.x=0;
			r.y=rebandY;
			...
			if(r.width<=0||r.x<0||r.width+r.x>img0.cols||r.height<=0||r.y<0||r.height+r.y>img0.rows)
			{
				return -1;
			}
			Mat p = img0(r);
			...
			else
			{
				...
				if(an<=0)
					return -1;					
				Mat element = getStructuringElement( MORPH_CROSS, Size( an, an ), Point( (an-1)/2, (an-1)/2) );
				if(colnum==0||colnum==1||colnum==4)
				{						
