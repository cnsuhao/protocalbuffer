...
/**
	...
	**graypic：是否是灰度图
*/
double realplate(CvSeq* plate,Mat gray,double *&cannyval,Mat resize,int count,int &nn,bool graypic)
{
	double max=0;
...
Mat extractEdge(Mat carplate0,Mat color_carplate0,int colnum,bool &whitebody)
{
		char sName[100]={0};
		int st=0;
		for(int k=0;k<carplate0.cols;k++)
		{
			for(int i=0;i<carplate0.rows/8;i++)
			{			
				st += carplate0.at<uchar>(i,k);
			}
		}
		if(st>255*(carplate0.rows/8)*(carplate0.cols)*0.5)//调参
			whitebody=1;
...
int  PlatePosition(   Mat &resize,Mat &frame,Mat &gray,CvSeq **OutPlate,vector <int> &nPlate ,int count,
...
void InitRect(	Rect *r	,int width , int height, int rete ,int flagi	)
{
	if(flagi==1)
	{
		if(r->x - r->width*0.25 > 0)
			r->x = r->x - r->width*0.25;
		else
			r->x = 0;
		if(r->y - r->height*0.25 > 0)
			r->y = r->y - r->height*0.25;
		else
			r->y = 0;
		if(r->x + r->width + r->width*0.4 < width)
			r->width = r->width + r->width*0.4;
		else
			r->width = width - r->x;
	...
	int &rete , vector<string>&Carplates , vector<string>&Platecolor ,vector<Rect>&Positions)
{
	...
	//********************定位开始****************
int iNumofPlate = PlatePosition(  resize,frame,gray,&plate ,nPlate,
...
if (iNumofPlate == 0)
{
	...
	for(int q=q0;q<pnum+q0;q++)
	{
		if(nPlate[q-q0]==-1)
			break;
		Rect* r = (Rect*) cvGetSeqElem(plate, nPlate[q]);
...
///////////////////倾斜校正结束=================================================================================
	...
	//	 continue; 
       //根据上面边缘图得到的ROI对车牌区域进行进一步细定位==================
		...
		{
			Rect CT_rect;
			CT_rect.x = color_carplate1.cols*0.2;//0.3
			CT_rect.y = color_carplate1.rows*0.2;//0.1
			CT_rect.width = color_carplate1.cols*0.4;//0.4
			CT_rect.height = color_carplate1.rows*0.5;
			Rect roi(CT_rect.x,CT_rect.y,CT_rect.width,CT_rect.height);
			Mat color_test2 = color_carplate1(roi);
...
bool CarPlateRecognition(char Filepath[], vector<string>&Carplates , vector<string>&Platecolor, vector<Rect> &Positions, bool model )
{
 ...
 { 
	// printf("这是第%d次循环\n",num);
	 ...
	 int k;
	 long HANDLE;
	  HANDLE = _findfirst( "D:\\测试\\车牌稳定测试\\好图\\888\\*.jpg", &file );
	  int count = 1 ;
	  cout << file.name << endl;

	  char path[256] ="D:\\测试\\车牌稳定测试\\好图\\888\\" ;
	  char savepath[256] ;
